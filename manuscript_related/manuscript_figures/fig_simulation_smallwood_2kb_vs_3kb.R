...
plot_dir <- "manuscript_related/manuscript_figures/simulation_smallwood_2kb_vs_3kb/"
...
methodName <- function(method) switch (method,
                                       'smallwood' = 'Smallwood 3kb (default)',
                                       'smallwood_2kb' = 'Smallwood 2kb')
...
png(here(plot_dir, "legend_methods.png"), width = 2500, height = 200, res = 300)
grid::grid.newpage()
...
plotDefaultSetting <- function(experiment) {
    ...
    getOneRow <- function(N, NP, sparseLevel) {
      ...
      smr0_dft <- smr0 %>% filter(
        (method == "smallwood" & threshold == 0.02) |
          (method == "smallwood_2kb" & threshold == 0.02) 
      )
  ...
  mySummarise <- function(col_name) {
    p <- smr_dft0 %>%
      ...
      geom_errorbar(aes(ymin = min, ymax = max), width = 0.3, position = position_dodge(width = 1)) +
      facet_wrap(~ N, nrow = 1) +
      scale_color_manual(values = COLORVALUES) + 
