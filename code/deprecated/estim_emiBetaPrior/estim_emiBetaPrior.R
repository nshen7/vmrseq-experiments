...
fwrite(smr_liu2021, "data/interim/estim_emiBetaPrior/emiBetaPrior_subtype_summary_liu2021.csv")
...
fwrite(smr_luo2017, "data/interim/estim_emiBetaPrior/emiBetaPrior_subtype_summary_luo2017.csv")

################################################################
# ==== Fit linear model on parameters ~ N cell ====
################################################################
smr_liu2021 <- fread("data/interim/estim_emiBetaPrior/emiBetaPrior_subtype_summary_liu2021.csv") %>%
  select(-c(Sample, GEO_accession)) %>%
  mutate(DataSource = "Liu2021")
smr_luo2017 <- fread("data/interim/estim_emiBetaPrior/emiBetaPrior_subtype_summary_luo2017.csv") %>%
...
fwrite(smr_long, "data/interim/estim_emiBetaPrior/emiBetaPrior_subtype_summary_with_subtypePDF.csv")
fwrite(smr_short, "data/interim/estim_emiBetaPrior/emiBetaPrior_subtype_summary_without_subtypePDF.csv")

smr_short <- fread("data/interim/estim_emiBetaPrior/emiBetaPrior_subtype_summary_without_subtypePDF.csv")
smr_long <- fread("data/interim/estim_emiBetaPrior/emiBetaPrior_subtype_summary_with_subtypePDF.csv")
...
smr_short %>%
  ...
  annotate("text", x = 250, y = 0.98, label = paste0('p-value = ', signif(lm$coef[2,4], 2))) +
  ylab("weight") + xlab("N cells") + ylim(0.8, 1)
ggsave("plots/estim_emiBetaPrior/point_fittedPrior_unmethPop_w1_vs_Ncell_27subtypes.png", width = 6, height = 5)
...
ggsave("plots/estim_emiBetaPrior/point_fittedPrior_unmethPop_param1_vs_Ncell_27subtypes.png", width = 6, height = 5)
...
smr_short %>%
  ...
  ylab(expression(beta[2])) + xlab("N cells") + ylim(1000,3000)
ggsave("plots/estim_emiBetaPrior/point_fittedPrior_unmethPop_param2_vs_Ncell_27subtypes.png", width = 6, height = 5)

smr_short %>%
  ggplot(aes(N_cell, l1_diff_u)) +
  geom_point(aes(color = DataSource, shape = CellClass), size = 2) +
  ylab("L1 difference") + xlab("N cells") + ylim(0, 1)
ggsave("plots/estim_emiBetaPrior/point_fittedPrior_unmethPop_L1Diff_vs_Ncell_27subtypes.png", width = 6, height = 5)
...
smr_short %>%
  ...
  annotate("text", x = 250, y = 0.98, label = paste0('p-value = ', signif(lm$coef[2,4], 2))) +
  ylab("weight") + xlab("N cells") + ylim(0.8, 1)
ggsave("plots/estim_emiBetaPrior/point_fittedPrior_methPop_w1_vs_Ncell_27subtypes.png", width = 6, height = 5)
...
ggsave("plots/estim_emiBetaPrior/point_fittedPrior_methPop_param1_vs_Ncell_27subtypes.png", width = 6, height = 5)
...
smr_short %>%
  ...
  annotate('text', x = 250, y = 2900, label = paste("italic(R)^{2}==", round(lm$r.squared, 2)), parse = TRUE) +
  annotate("text", x = 250, y = 2780, label = paste0('p-value = ', signif(lm$coef[2,4], 2))) +
  ylab(expression(alpha[2])) + xlab("N cells") + ylim(1000,3000)
ggsave("plots/estim_emiBetaPrior/point_fittedPrior_methPop_param2_vs_Ncell_27subtypes.png", width = 6, height = 5)

smr_short %>%
  ggplot(aes(N_cell, l1_diff_m)) +
  geom_point(aes(color = DataSource, shape = CellClass), size = 2) + 
  ylab("L1 difference") + xlab("N cells") + ylim(0, 1)
ggsave("plots/estim_emiBetaPrior/point_fittedPrior_methPop_L1Diff_vs_Ncell_27subtypes.png", width = 6, height = 5)
...
ggsave("plots/estim_emiBetaPrior/point_unmethPop_predicted_vs_original_L1Diff_27subtypes.png", width = 6, height = 5)
...
smr_short_pred_m %>%
  ...
  xlab("L1 difference (original)") + ylab("L1 difference (predicted)") + 
  xlim(0,1.1) + ylim(0, 1.1)
ggsave("plots/estim_emiBetaPrior/point_methPop_predicted_vs_original_L1Diff_27subtypes.png", width = 6, height = 5)
