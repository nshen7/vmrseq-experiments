...
wrapper1 <- function(info_row, cores = 16){
  ...
  mean_m <- mean(mf_m[mf_m<1]); var_m <- var(mf_m[mf_m<1])
...
smr %>%
  ...
  geom_smooth(method = "lm", formula = y~I(1/x), linetype = 4, se = F, size = 0.7, color = "pink3") +
  geom_point(aes(color = DataSource, shape = CellClass), size = 2) +
  annotate("text", x = annot_x, y = 5, label = paste0('y~log(x): R^2 = ', round(loglm$r.squared, 3)), color = "skyblue4") +
  annotate('text', x = annot_x, y = 5.3, label = paste0("y~x: R^2 = ", round(lm$r.squared, 3)), color = "sienna2") +
  annotate("text", x = annot_x, y = 5.6, fontface = 2, label = paste0('y~1/x: R^2 = ', round(invlm$r.squared, 3)), color = "pink3") +
  ylab("Alpha") + xlab("N cells") + ylim(5, 12.2)
...
smr %>%
  ...
  geom_smooth(method = "lm", formula = y~I(1/x), linetype = 4, se = F, size = 0.7, color = "pink3") +
  geom_point(aes(color = DataSource, shape = CellClass), size = 2) +
  annotate("text", x = annot_x, y = 0.1, label = paste0('y~log(x): R^2 = ', round(loglm$r.squared, 3)), color = "skyblue4") +
  annotate('text', x = annot_x, y = 0.25, label = paste0("y~x: R^2 = ", round(lm$r.squared, 3)), color = "sienna2") +
  annotate("text", x = annot_x, y = 0.4, fontface = 2, label = paste0('y~1/x: R^2 = ', round(invlm$r.squared, 3)), color = "pink3") +
  ylab("Beta") + xlab("N cells") + ylim(0, 3)
...
#######################################################################
# ==== Fit linear model on parameters ~ median across-cell coverge ====
#######################################################################
...
## plotting parameter
annot_x <- ifelse(nrow(smr)==27, 20, 25) 

## fitted parameters for unmeth subpop vs. Median across-cell covergae
(loglm <- summary(lm(w_u ~ log(med_cov), data = smr)))
(invlm <- summary(lm(w_u ~ I(1/med_cov), data = smr)))
(lm <- summary(lm(w_u ~ med_cov, data = smr)))
smr %>%
  ggplot(aes(med_cov, w_u)) +
  geom_smooth(method = "lm", formula = y~log(x), se = F, size = 0.7, color = "skyblue4") +
  geom_smooth(method = "lm", se = F, linetype = 2, size = 0.7, color = "sienna2") +
  geom_smooth(method = "lm", formula = y~I(1/x), linetype = 4, se = F, size = 0.7, color = "pink3") +
  geom_point(aes(color = DataSource, shape = CellClass), size = 2) +
  ...
  annotate('text', x = annot_x, y = 0.06, label = paste0("y~x: R^2 = ", round(lm$r.squared, 3)), color = "sienna2") +
  annotate("text", x = annot_x, y = 0.10, label = paste0('y~1/x: R^2 = ', round(invlm$r.squared, 3)), color = "pink3") +
  ylab("Prob mass at 0") + xlab("Median across-cell covergae") + ylim(0,1)
ggsave(paste0("plots/estim_emiBetaPrior_inflated/point_fittedPrior_unmethPop_w_vs_medCov_",nrow(smr),"subtypes.png"), width = 6, height = 5)

(loglm <- summary(lm(alpha_u ~ log(med_cov), data = smr)))
(invlm <- summary(lm(alpha_u ~ I(1/med_cov), data = smr)))
(lm <- summary(lm(alpha_u ~ med_cov, data = smr)))
smr %>%
  ggplot(aes(med_cov, alpha_u)) +
  geom_smooth(method = "lm", formula = y~log(x), se = F, size = 0.7, color = "skyblue4") +
  geom_smooth(method = "lm", se = F, linetype = 2, size = 0.7, color = "sienna2") +
  geom_smooth(method = "lm", formula = y~I(1/x), linetype = 4, se = F, size = 0.7, color = "pink3") +
  geom_point(aes(color = DataSource, shape = CellClass), size = 2) +
  ...
  annotate('text', x = annot_x, y = 0.3, label = paste0("y~x: R^2 = ", round(lm$r.squared, 3)), color = "sienna2") +
  annotate("text", x = annot_x, y = 0.45, fontface = 2, label = paste0('y~1/x: R^2 = ', round(invlm$r.squared, 3)), color = "pink3") +
  ylab("Alpha") + xlab("Median across-cell covergae") + ylim(0, 3)
ggsave(paste0("plots/estim_emiBetaPrior_inflated/point_fittedPrior_unmethPop_alpha_vs_medCov_",nrow(smr),"subtypes.png"), width = 6, height = 5)

(loglm <- summary(lm(beta_u ~ log(med_cov), data = smr)))
(invlm <- summary(lm(beta_u ~ I(1/med_cov), data = smr)))
(lm <- summary(lm(beta_u ~ med_cov, data = smr)))
smr %>%
  ggplot(aes(med_cov, beta_u)) +
  geom_smooth(method = "lm", formula = y~log(x), se = F, size = 0.7, color = "skyblue4") +
  geom_smooth(method = "lm", se = F, linetype = "dashed", size = 0.7, color = "sienna2") +
  geom_smooth(method = "lm", formula = y~I(1/x), linetype = 4, se = F, size = 0.7, color = "pink3") +
  geom_point(aes(color = DataSource, shape = CellClass), size = 2) +
  ...
  annotate('text', x = annot_x, y = 3.3, label = paste0("y~x: R^2 = ", round(lm$r.squared, 3)), color = "sienna2") +
  annotate("text", x = annot_x, y = 3.6, fontface = 2, label = paste0('y~1/x: R^2 = ', round(invlm$r.squared, 3)), color = "pink3") +
  ylab("Beta") + xlab("Median across-cell covergae") + ylim(3, 9)
ggsave(paste0("plots/estim_emiBetaPrior_inflated/point_fittedPrior_unmethPop_beta_vs_medCov_",nrow(smr),"subtypes.png"), width = 6, height = 5)

## fitted parameters for meth subpop vs. Median across-cell covergae
(loglm <- summary(lm(w_m ~ log(med_cov), data = smr)))
(invlm <- summary(lm(w_m ~ I(1/med_cov), data = smr)))
(lm <- summary(lm(w_m ~ med_cov, data = smr)))
smr %>%
  ggplot(aes(med_cov, w_m)) +
  geom_smooth(method = "lm", formula = y~log(x), se = F, size = 0.7, color = "skyblue4") +
  geom_smooth(method = "lm", se = F, linetype = "dashed", size = 0.7, color = "sienna2") +
  geom_smooth(method = "lm", formula = y~I(1/x), linetype = 4, se = F, size = 0.7, color = "pink3") +
  geom_point(aes(color = DataSource, shape = CellClass), size = 2) +
  ...
  annotate('text', x = annot_x, y = 0.06, label = paste0("y~x: R^2 = ", round(lm$r.squared, 3)), color = "sienna2") +
  annotate("text", x = annot_x, y = 0.10, label = paste0('y~1/x: R^2 = ', round(invlm$r.squared, 3)), color = "pink3") +
  ylab("Prob mass at 1") + xlab("Median across-cell covergae") + ylim(0, 1)
ggsave(paste0("plots/estim_emiBetaPrior_inflated/point_fittedPrior_methPop_w_vs_medCov_",nrow(smr),"subtypes.png"), width = 6, height = 5)

(loglm <- summary(lm(alpha_m ~ log(med_cov), data = smr)))
(invlm <- summary(lm(alpha_m ~ I(1/med_cov), data = smr)))
(lm <- summary(lm(alpha_m ~ med_cov, data = smr)))
smr %>%
  ggplot(aes(med_cov, alpha_m)) +
  geom_smooth(method = "lm", formula = y~log(x), se = F, size = 0.7, color = "skyblue4") +
  geom_smooth(method = "lm", se = F, linetype = "dashed", size = 0.7, color = "sienna2") +
  geom_smooth(method = "lm", formula = y~I(1/x), linetype = 4, se = F, size = 0.7, color = "pink3") +
  geom_point(aes(color = DataSource, shape = CellClass), size = 2) +
  ...
  annotate('text', x = annot_x, y = 5.3, label = paste0("y~x: R^2 = ", round(lm$r.squared, 3)), color = "sienna2") +
  annotate("text", x = annot_x, y = 5.6, fontface = 2, label = paste0('y~1/x: R^2 = ', round(invlm$r.squared, 3)), color = "pink3") +
  ylab("Alpha") + xlab("Median across-cell covergae") + ylim(5, 12.2)
ggsave(paste0("plots/estim_emiBetaPrior_inflated/point_fittedPrior_methPop_alpha_vs_medCov_",nrow(smr),"subtypes.png"), width = 6, height = 5)

(loglm <- summary(lm(beta_m ~ log(med_cov), data = smr)))
(invlm <- summary(lm(beta_m ~ I(1/med_cov), data = smr)))
(lm <- summary(lm(beta_m ~ med_cov, data = smr)))
smr %>%
  ggplot(aes(med_cov, beta_m)) +
  geom_smooth(method = "lm", formula = y~log(x), se = F, size = 0.7, color = "skyblue4") +
  geom_smooth(method = "lm", se = F, linetype = "dashed", size = 0.7, color = "sienna2") +
  geom_smooth(method = "lm", formula = y~I(1/x), linetype = 4, se = F, size = 0.7, color = "pink3") +
  geom_point(aes(color = DataSource, shape = CellClass), size = 2) +
  ...
  annotate('text', x = annot_x, y = 0.25, label = paste0("y~x: R^2 = ", round(lm$r.squared, 3)), color = "sienna2") +
  annotate("text", x = annot_x, y = 0.4, fontface = 2, label = paste0('y~1/x: R^2 = ', round(invlm$r.squared, 3)), color = "pink3") +
  ylab("Beta") + xlab("Median across-cell covergae") + ylim(0, 3)
ggsave(paste0("plots/estim_emiBetaPrior_inflated/point_fittedPrior_methPop_beta_vs_medCov_",nrow(smr),"subtypes.png"), width = 6, height = 5)

smr %>%
  ggplot(aes(med_cov, mean_u)) +
  geom_point(aes(color = DataSource, shape = CellClass), size = 2) 
smr %>%
  ggplot(aes(med_cov, var_u)) +
  geom_point(aes(color = DataSource, shape = CellClass), size = 2) 

smr %>%
  ggplot(aes(med_cov, mean_m)) +
  geom_point(aes(color = DataSource, shape = CellClass), size = 2) 
smr %>%
  ggplot(aes(med_cov, var_m)) +
  geom_point(aes(color = DataSource, shape = CellClass), size = 2) 
