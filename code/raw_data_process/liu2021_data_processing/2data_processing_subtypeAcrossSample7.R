...
# for (i in 12:13) wrapper(subtype_smr_sel$SubType[i], mc.cores = 10) # 3826301.pbsha.ib.sockeyee
# for (i in 3) wrapper(subtype_smr_sel$SubType[i], chr_names = paste0("chr", 9:19), mc.cores = 6) # 3829878.pbsha.ib.sockeye

# subtype_dir <- "data/processed/summarized_liu2021/summarized_subtype_CT-L6_Il1rap_4360cells"
# part1 <- fread(paste0(subtype_dir, "_chr1_to_chr8.csv.gz"))
# part2 <- fread(paste0(subtype_dir, "_chr9_to_chr19.csv.gz"))
# fwrite(rbind(part1, part2), file = paste0(subtype_dir, ".csv"), quote = F)
# R.utils::gzip(paste0(subtype_dir, ".csv"), remove = T, overwrite = T)
# if (file.exists(paste0(subtype_dir, ".csv.gz"))) {
#   unlink(paste0(subtype_dir, "_chr1_to_chr8.csv.gz"))
#   unlink(paste0(subtype_dir, "_chr9_to_chr19.csv.gz"))
# }

# subtype_dir <- "data/processed/summarized_liu2021/summarized_subtype_DG_dg-all_6178cells"
# part1 <- fread(paste0(subtype_dir, "_chr1_to_chr8.csv.gz"))
# part2 <- fread(paste0(subtype_dir, "_chr9_to_chr19.csv.gz"))
# fwrite(rbind(part1, part2), file = paste0(subtype_dir, ".csv"), quote = F)
# R.utils::gzip(paste0(subtype_dir, ".csv"), remove = T, overwrite = T)
...
subtype_dir <- "data/processed/summarized_liu2021/summarized_subtype_IT-L23_Cux1_6550cells"
part1 <- fread(paste0(subtype_dir, "_chr1_to_chr8.csv.gz"))
part2 <- fread(paste0(subtype_dir, "_chr9_to_chr19.csv.gz"))
fwrite(rbind(part1, part2), file = paste0(subtype_dir, ".csv"), quote = F)
R.utils::gzip(paste0(subtype_dir, ".csv"), remove = T, overwrite = T)
