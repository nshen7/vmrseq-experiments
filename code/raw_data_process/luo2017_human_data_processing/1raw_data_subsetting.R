...
metadata <- fread("data/metadata/metadata_luo2017/NIHMS893063-supplement-Table_S2_csv.csv", skip = 1, header = T)
all(metadata$Sample %in% folder_list_modified) # = TRUE

folder_list_human <- folder_list[which(folder_list_modified %in% metadata$Sample)]

for (cell_folder_name in folder_list_human[1027:length(folder_list_human)]) {
