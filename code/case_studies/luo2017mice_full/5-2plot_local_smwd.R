...
plot_dir <- "plots/case_studies/luo2017mice_full/comparison/cell_type_regional_methylation_smwd/"
if (!file.exists(plot_dir)) dir.create(plot_dir)
md <- fread("data/metadata/metadata_luo2017/sample_info_processed.csv")
se <- loadHDF5SummarizedExperiment(paste0(read_dir, "smallwood_regionSummary_vmrs"))
colData(se) <- DataFrame(md)

plotCellTypeMeth <- function(top_i) {
  # index of the top ith region in se
  idx <- order(granges(se)$var_lb, decreasing = T)[top_i]
  gr <- granges(se)[idx]
