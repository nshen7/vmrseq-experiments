...
plot_dir <- "plots/case_studies/luo2017mice_full/comparison/cell_type_regional_methylation_scmet/"
if (!file.exists(plot_dir)) dir.create(plot_dir)
md <- fread("data/metadata/metadata_luo2017/sample_info_processed.csv")
se <- loadHDF5SummarizedExperiment(paste0(read_dir, "scmet_regionSummary_vmrs"))
colData(se) <- DataFrame(md)

plotCellTypeMeth <- function(top_i) {
  # index of the top ith region in se
  idx <- order(granges(res_region$scmet)$gamma, decreasing = T)[top_i]
  gr <- granges(res_region$scmet)[idx]
  # summarize phenotypic info and methylation value per cell type
  df <- data.frame(
    colData(se),
    MF = assays(res_region$scmet)$M[idx,] / assays(res_region$scmet)$Cov[idx,]
  ) %>% 
    filter(!is.na(MF))
