...
# ---- Bash arguments ----
p <- ArgumentParser(description = 'Run vmrseq on Argelaguet2019 full dataset')
p$add_argument('--chr', type = "integer", help = 'Chromosome number')
args <- p$parse_args(commandArgs(TRUE))
chr <- as.character(args$chr)

# ---- main ----
read_dir <- here('data', 'interim', 'case_studies', 'argelaguet2019_full', 'met', 'vmrseq', 'input')
write_dir <- here('data', 'interim', 'case_studies', 'argelaguet2019_full', 'met', 'vmrseq', 'output')
