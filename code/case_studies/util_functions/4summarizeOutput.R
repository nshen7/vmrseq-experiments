...
summarizeOutputRegion <- function(read_dir, write_dir, methods, chr_names = paste0('chr', 1:19)) {
  ...
  ### smallwood 2kb windows
  if ("smallwood_2kb" %in% methods) {
    all_smwd.gr <- readRDS(paste0(read_dir, "smallwood_2kb/output/smallwood_output_varLowerBound.rds"))
    ...
    saveHDF5SummarizedExperiment(rs_smwd.se, here(write_dir, "smallwood_2kb_regionSummary_vmrs"), replace=TRUE)
  }
